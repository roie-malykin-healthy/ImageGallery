...
//  ImageGallery
//
//  Created by Roie Malykin on 24/05/202
...
extension UIImage {
    ...
    // swiftlint:disable implicit_singleton_dependency
    ...
    // swiftlint:enable implicit_singleton_dependency
...
extension NSMutableAttributedString {
    func setFont(_ newValue: UIFont?) {
        if newValue != nil {
            ...
            addAttributes([.font: newValue!], range: NSRange(location: 0, length: length))
        }
    }
}

extension UIFont {
    func scaled(by factor: CGFloat) -> UIFont { withSize(pointSize * factor) }
}
